...
with col_logo:
    if os.path.exists(NOMBRE_ARCHIVO_LOGO):
        st.image(NOMBRE_ARCHIVO_LOGO, width=120)
    else:
        st.warning("Suba 'logo.png'")

with col_titulo:
    st.markdown(
        "<h1 class='header-title'>Crece & CrediK</h1>", unsafe_allow_html=True
    )
    st.markdown(
...
with st.expander("Filtros y Calculadora de Préstamos", expanded=True):
    ...
    with col_ctrl2:
        filtro_buscar = st.text_input(
            "Buscar Cliente por Nombre:", placeholder="Escriba aquí para buscar..."
        )

    with col_ctrl3:
        st.write("Calculadora Rápida:")
        monto_sim = st.number_input(
            "Monto a Calcular ($):", value=1000.0, step=500.0
        )
        pago_semanal_sim = (monto_sim * FACTOR_INTERES) / SEMANAS_TOTALES
        st.caption(
            f"Pago semanal: **${pago_semanal_sim:,.2f}** a {SEMANAS_TOTALES} semanas."
        )

# --- PESTAÑAS PRINCIPALES ---
tab_captura, tab_resumen, tab_fichas = st.tabs(
    ["Captura y Pagos", "Resumen de Saldos", "Fichas e INE"]
)
...
with tab_captura:
    ...
    if st.button("Agregar Nuevo Cliente"):
        ...
        st.session_state.df_clientes = pd.concat(
            [st.session_state.df_clientes, pd.DataFrame([nuevo_registro])],
            ignore_index=True,
        )
        st.rerun()
...
with tab_resumen:
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Ganancia Empresa": st.column_config.NumberColumn(
                "Ganancia Empresa", format="$%.2f"
            ),
            "Monto Entregado": st.column_config.NumberColumn(
                "Monto Entregado", format="$%.2f"
            ),
