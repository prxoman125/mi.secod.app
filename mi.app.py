...
st.write(
    "Ajusta los parámetros del tiro y del portero para calcular el tiempo de"
    " reacción, la dirección y la probabilidad de atajar el balón."
)
...
ancho_porteria = st.sidebar.slider(
    "Ancho de la portería (m)", 3.0, 7.32, 7.32, step=0.1
)
alto_porteria = st.sidebar.slider(
    "Alto de la portería (m)", 1.5, 2.44, 2.44, step=0.05
)

# 2. Datos del Portero
st.sidebar.subheader("🧤 Portero")
altura_portero = st.sidebar.slider(
    "Altura del portero (m)", 1.50, 2.10, 1.85, step=0.01
)

# 3. Datos del Tiro
st.sidebar.subheader("🎯 Tiro")
distancia_tiro = st.sidebar.slider(
    "Distancia del tiro (m)", 9.15, 30.0, 11.0, step=0.5
)  # 11m es el punto penal
velocidad_kmh = st.sidebar.slider(
    "Velocidad del balón (km/h)", 40, 140, 90, step=5
)

# Dirección del tiro
st.sidebar.markdown("**Ubicación del Tiro en la Portería:**")
pos_x = st.sidebar.slider(
    "Posición horizontal X (0 = Izquierda, 100 = Derecha)", 0, 100, 50
)
pos_y = st.sidebar.slider(
    "Posición vertical Y (0 = Suelo, 100 = Travesaño)", 0, 100, 50
)
...
with col1:
  ...
  st.metric(
      ...
      value=f"{velocidad_ms:.1f} m/s ({velocidad_kmh} km/h)",
  )
